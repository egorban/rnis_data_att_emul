...
load_atts()->
	LoadNode = application:get_env(rnis_data_att_emul,load_node, ?LOAD_NODE),
 	...
 	[E||{Id,Name}=E<-[{catch binary_to_integer(Id),Name}||{<<_:5/binary,Id/binary>>,{Name,_}}<-[{Id,proplists:get_value(Prefix,?PREFIX)}||[{Prefix,Id}]<-Atts],Name=/=undefined],is_integer(Id)].